    ...
    <div className="space-y-6 bg-muted py-6">
      ...
      <section className="">
        <div className="max-w-4xl mx-auto grid text-lg justify-center gap-4 px-4 text-foreground">
          <h1 className="text-center uppercase text-4xl font-bold">Pastors / Elders</h1>
          ...
          <span>
            ...
            <BibleRef content="Acts 20:17, 28; Titus 1:5-9; 1 Peter 5:1-4" size={4} />
            Our pastors/elders/overseers are the under-shepherds who serve the Chief Shepherd (i.e. Jesus) by leading and caring for the flock of Olivet Baptist Church. Our pastors/elders/overseers
            are accountable to the church as a whole.
          </span>
        </div>
      </section>
          ...
          {staffInfo.pastors.map((pastor) => (
            <div key={pastor.name} className="max-w-fit mt-auto">
              ...
              <div className="relative">
                <Image src={pastor.img} alt={pastor.name} width={300} height={300} className="rounded-md drop-shadow-lg shadow" />
      ...
      <section className="max-w-4xl mx-auto grid text-lg justify-center gap-4 py-10">
        <h1 className="text-center uppercase text-3xl font-bold">Ministry Staff</h1>
        ...
        <div className="flex justify-center flex-wrap gap-4 mx-4">
          {staffInfo.staff.map((staff) => (
            <div key={staff.name} className="max-w-fit mt-auto">
              ...
              <div className="relative">
                <Image src={staff.img} alt={staff.name} width={300} height={300} className="rounded-md" />
                {staff.title && <p className="absolute bottom-0 w-full bg-foreground py-1.5 text-center text-muted tracking-wider text-lg border border-foreground/30 rounded-b-md">{staff.title}</p>}
              </div>
            </div>
          ))}
        </div>
